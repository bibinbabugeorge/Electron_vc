...
function EditGroupInit(participant) {
  var list = "";
...
function AddToGroup(userid) {
  ...
  if (!userobj) {
    userobj = submitUserList.find(o => o.userid === userid);
  }
...
function Bindparticipant(participant) {
  ...
  $('#memCount').text(participant.length - 1 + " Members")
...
function bindGroupData(RoomDetails) {
  roomid = RoomDetails.roomid;
  $("#groupName").val(RoomDetails.name);
  submitUserList = RoomDetails.joinedusers;
